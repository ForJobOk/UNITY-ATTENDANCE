...
public class Calendar : MonoBehaviour
{
    ...
    private  FinishTimeText finishTimeText; //終了した日付を取得するため

    public void BtnDayViewEvents(Button btn) //カレンダーの日付を押したら...
    {
        ...
        */
        SceneManager.LoadScene("WorkTimeScene");
        /*
        if (day == finishTimeText.dateNumberFinish)
        {

            /*StartTimeText stt = GetComponent<StartTimeText>(); 　//別のスクリプトからメソッドを呼び出す

            stt.StartTime();

            FinishTimeText ftt = GetComponent<FinishTimeText>();  //別のスクリプトからメソッドを呼び出す
